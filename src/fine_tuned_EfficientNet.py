...
class FineTunedEfficientNet:
    ...
    def __load_pretrained_model(input_shape):
        # defining a CNN using the EfficientNetB0 architecture pre-trained on the ImageNet dataset
        pretrained_model = tf.keras.applications.EfficientNetB0(input_shape=list(input_shape),
                                                                include_top=False,
                                                                # weights='imagenet',
                                                                weights="src/weights/efficientnet_b0_weights.h5")
    ...
    def run_fine_tuned_vit(self, data_object):
        if self.phase == "train":
